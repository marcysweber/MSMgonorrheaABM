...
// without
// a clear in between!!!
public class ThreadSafeScheduleGroup extends ScheduleGroup {
   ...
   */
  protected ThreadSafeActionList actions = new ThreadSafeActionList(new ThreadSafeRandomHelper(1));
  /**
   * List of actions whose priority is random.
   */
  protected ThreadSafeActionList randomActions = new ThreadSafeActionList(new ThreadSafeRandomHelper(1));
  /**
   * List of actions that should be executed first.
   */
  protected ThreadSafeActionList firstActions = new ThreadSafeActionList(new ThreadSafeRandomHelper(1));
  
  /**
   * List of actions that are executed as last actions but immediately
   * prior to those.
   */
  protected ThreadSafeActionList firstOfLastActions = new ThreadSafeActionList(new ThreadSafeRandomHelper(1));
  
  /**
   * List of actions that should be executed last.
   */
  protected ThreadSafeActionList lastActions = new ThreadSafeActionList(new ThreadSafeRandomHelper(1));
  ...
  public void sort() {
    ...
    if (actions.size() > 0) {
      ...
      Uniform defaultUniform = RandomHelper.createUniform(0.0,0.0);
