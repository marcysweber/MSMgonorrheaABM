...
public ThreadSafeRandomHelper registerDistributions() {
	//set seed and random distributions
			...
			Normal screenIntervalMSWNormal = new Normal(parameters.getDouble("screen_interval_MSW")*52, 52*parameters.getDouble("screen_interval_MSW")/10, eng);
			randomHelper.registerDistribution("screenIntervalMSWNormal", screenIntervalMSWNormal);
			Uniform screenFirstValueMSWUniform = new Uniform(0, parameters.getDouble("screen_interval_MSW")*52, eng);
			randomHelper.registerDistribution("screenFirstValueMSWUniform", screenFirstValueMSWUniform);
			
			Normal screenIntervalWNormal = new Normal(parameters.getDouble("screen_interval_W")*52, 52*parameters.getDouble("screen_interval_W")/10, eng);
			randomHelper.registerDistribution("screenIntervalWNormal", screenIntervalWNormal);
			Uniform screenFirstValueWUniform = new Uniform(0, parameters.getDouble("screen_interval_W")*52, eng);
			randomHelper.registerDistribution("screenFirstValueWUniform", screenFirstValueWUniform);
			...
			Exponential delayToSeekCareMSWExp = new Exponential(1/parameters.getDouble("delay_to_seek_care_msw"), eng);
			randomHelper.registerDistribution("delayToSeekCareMSWExp", delayToSeekCareMSWExp);
			

			Exponential delayToSeekCareFExp = new Exponential(1/parameters.getDouble("delay_to_seek_care_f"), eng);
			randomHelper.registerDistribution("delayToSeekCareFExp", delayToSeekCareFExp);
			...
			Exponential delayToRetreatmentMSWExp = new Exponential(1/parameters.getDouble("delay_to_retreatment_msw"), eng);
			randomHelper.registerDistribution("delayToRetreatmentMSWExp", delayToRetreatmentMSWExp);
			
			Exponential delayToRetreatmentFExp = new Exponential(1/parameters.getDouble("delay_to_retreatment_f"), eng);
			randomHelper.registerDistribution("delayToRetreatmentFExp", delayToRetreatmentFExp);
...
public void infectInitialInfected(int InfectiousCount, int IndivCount, Population population) {
	...
	double amountToInfectMSW = (InfectiousCount/(double)IndivCount) * population.mswCount();
	List <Object> MSWtoInfect = population.mswList.stream().limit((long) amountToInfectMSW).collect(Collectors.toList());
	indivToInfectList.addAll(MSWtoInfect);
	
	double amountToInfectW = (InfectiousCount/(double)IndivCount) * population.wCount();
	List <Object> WtoInfect = population.wList.stream().limit((long) amountToInfectW).collect(Collectors.toList());
